...
bool User::operator<(const User& u1) const {
	//TODO:
    int number_lectures1=0, number_lectures2=0;
    if(titleReading.first!="") number_lectures1++;
    number_lectures1+=titlesRead.size();
    if(u1.getReading().first!="") number_lectures2++;
    number_lectures2+=u1.getReadings().size();
	return number_lectures1<number_lectures2;
}
