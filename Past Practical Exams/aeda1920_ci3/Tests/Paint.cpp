...
void Paint::setTitle(string ntitle) {
    title=ntitle;
}
...
bool PaintCatalogItem::operator<(const PaintCatalogItem& pci) const {
    if (getYear()>pci.getYear()) return true;
        ...
        if (getAuthor()<pci.getAuthor()) return true;
        else if(getAuthor()==pci.getAuthor()){
            if (getTitle()<pci.getTitle()) return true;
        }
    }
    return false;
}

void PaintCatalogItem::setTitle(string ntitle) {
    paintItem->setTitle(ntitle);
}
